    ...
    public static function order_meta_dropdown_list(){
    	...
    	if(isset($_GET['page'])){
    		if($_GET['page'] === "wf_woocommerce_packing_list_invoice"){
		        ...
		        if ( count( $order_meta_query_arr ) ) {
		            foreach ( $order_meta_query_arr as $meta ) {
		            	if("" !== trim($meta->meta_key)){
		            		$order_meta_query[] = array('label' => $meta->meta_key);
		            		$order_metas_arr[] = $meta->meta_key;
		            	}
		            }
		        }
		        if(!in_array($first_meta_key,$order_metas_arr)){
		        	$order_meta_query = array_merge($order_meta_query,$user_added_arr);
		        }
    		}
    	}
